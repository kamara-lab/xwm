"""Where downloaded datasets live, and how they get there.

This is the first download in the library. :mod:`xwm.envs` needs one too, for
the Franka URDF, but it delegates that entirely to Newton's own asset fetcher;
nothing here existed before, so the rules are set here.

Three of them:

* **One directory, and the user names it.** ``$XWM_DATA_HOME`` if set, else
  ``~/.cache/xwm/datasets``. A dataset cache belongs where the user's other
  caches are, not beside their source tree, and never inside the package.
...
def cache_dir(*parts: str) -> Path:
    """The dataset cache directory, created if absent.
...
def require(modules: str | Iterable[str], extra: str = "data") -> tuple[Any, ...]:
    ...
    *call* time with an actionable message, so importing :mod:`xwm` never needs
    ...
    for name in names:
        try:
            ...
            raise ImportError(f"this reader needs {name}; install it with {hint}") from exc
